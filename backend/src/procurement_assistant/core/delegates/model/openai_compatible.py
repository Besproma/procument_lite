...
from procurement_assistant.core.config.settings import ModelDelegateSettings
...
class OpenAICompatibleModelDelegate(ModelDelegate):
    ...
    def __init__(
        self,
        settings: ModelDelegateSettings,
        prompts: PromptProvider,
        *,
        scenario_router_task_id: str,
    ) -> None:
        """创建模型客户端，但不读取任何采购业务模块。

        ``prompts`` 和 ``scenario_router_task_id`` 都由 Business 在启动时显式传入。
        Core 因而只负责“怎样调用模型”，不知道有哪些具体模型任务。
        """

        self._settings = settings
        self._prompts = prompts
        self._scenario_router_task_id = scenario_router_task_id
        self._primary = self._create_chat_model(
            base_url=settings.primary.base_url,
            model_name=settings.primary.model_name,
            api_key=settings.primary.api_key,
        )
        ...
        if settings.fallback is not None:
            self._fallback = self._create_chat_model(
                base_url=settings.fallback.base_url,
                model_name=settings.fallback.model_name,
                api_key=settings.fallback.api_key,
            )
        ...
        *,
        ...
        try:
            ...
            if _is_retryable_model_error(exc):
                raise RetryableDelegateError(f"模型任务 {task_id} 暂时不可用") from exc
            if isinstance(exc, ValueError):
                raise DelegateContractError(f"模型任务 {task_id} 返回结构不合法") from exc
            # 其余 4xx、鉴权、业务拒绝和无法分类的 SDK 错误不自动重放。错误正文、URL
            # 和凭据只保留在受控日志链中，不进入 Graph 或用户响应。
            raise NonRetryableDelegateError(f"模型任务 {task_id} 执行失败") from exc
        ...
        *,
        ...
        try:
            ...
            agent = create_agent(
                ...
                system_prompt=self._prompts.get(self._scenario_router_task_id),
            )
            memory_text = (
