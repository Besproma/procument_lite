...
from procurement_assistant.core.delegates.common.stream_events import AgentStreamEvent
from procurement_assistant.core.domain.identifiers import new_identifier
...
class AGUIEventEmitter:
    ...
    async def custom(
        ...
        """发出一个带通用信封的 ``CUSTOM`` 事件。

        事件信封由这里统一生成，业务节点只提供已经通过 Pydantic 校验的 payload，不能
        自行修改 schema、threadId、runId 或 sequence。
        """
