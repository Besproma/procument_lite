...
class ExecutionContext:
    ...
    async def call_delegate(
        ...
        *,
        ...
        for attempt in range(1, self.settings.delegate_max_attempts + 1):
            self.deadline.ensure_remaining()
            timeout_seconds = min(
                self.settings.delegate_attempt_timeout_seconds,
            ...
            try:
                ...
                last_error = exc
                if attempt >= self.settings.delegate_max_attempts:
                    raise
