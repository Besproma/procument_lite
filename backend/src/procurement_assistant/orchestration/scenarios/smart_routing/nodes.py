...
from procurement_assistant.delegates.agents.column_recognition import ColumnRecognitionDelegate
...
from procurement_assistant.domain.errors import InvalidUserInputError, ProcurementAssistantError
...
class SmartRoutingNodes:
    ...
    async def wait_for_custom_purchase(
        ...
        resumed = interrupt(state.wait_request.model_dump(mode="json"))
        command = GraphResumeInput.model_validate(resumed)
        if command.operation != ActionOperation.GO_CUSTOM_PURCHASE:
