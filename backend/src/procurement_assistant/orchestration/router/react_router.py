...
class ReactScenarioRouter:
    ...
    async def route(
        ...
        *,
        ...
        descriptions = tuple(item.model_description() for item in self._catalog.values())
        ...
        if result.scenario_id is not None and result.scenario_id not in self._catalog:
